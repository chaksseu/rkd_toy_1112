...
WANDB_NAME=f"1010_rkd_xt_b512_ddim_full_init_student_H_with_diffW0.01_randN"
# WANDB_NAME=f"1010_ony_diff_continue_1e5_student_H"
...
CONFIG = {
    ...
    # "student_init_ckpt": "",                     
    "student_init_ckpt": "runs/1010_ony_diff_continue_1e5_student_H/ckpt_student_step090000.pt",                     
    ...
    # diffusion loss 가중치
    "W_DIFF": 0.01,                               # ε-pred MSE 가중치
    "W_RKD": 1,
    "rkd_ddim_steps_to_t": 50,   # t_sel까지 최대 몇 번의 DDIM 전이만 사용할지

    "batch_size": 512,
    "num_noises": 8192, 
    "epochs_total": 2000000,          # 총 스텝 수 (기존 epochs_per_stage 대신 사용)
    ...
    # wandb
    "use_wandb": True,
    "wandb_project": "RKD-DKDM-AICA-1010",
    "wandb_run_name": WANDB_NAME,

    "use_learnable_H": True,
    # "H_init": [1,1,1,  1,1,1,  1,1,1],     # 초기값(I). 원하면 너의 H로 바꿔도 OK
    "H_init": [1,0,0,  0,1,0,  0,0,1],     # 초기값(I). 원하면 너의 H로 바꿔도 OK
    "H_eps": 1e-6,                        # w 분모 안정화
    "W_H_REG": 0.0,                       # (옵션) w 안정화/정규화 가중치
    "resume_H_ckpt": "",                  # (옵션) H만 재개 로드 경로
    "gt_inv_H": [
        [ 0.00885684860968074,   0.01658084449021627,  -3.501544799176107 ],
        [-0.05767250257466529,   0.034397528321318226, -0.9268795056642634],
        [-0.00012873326467559217,-0.0008341915550978372, 1.0 ]
    ],
}
...
class LearnableHomography(nn.Module):
    ...
    """
    def __init__(self, init_9=None, eps: float = 1e-6):
        super().__init__()
        if init_9 is None:
            init = torch.eye(3, dtype=torch.float32)
        else:
            init = torch.tensor(init_9, dtype=torch.float32).view(3,3)
        self.H = nn.Parameter(init)    # learnable 3x3
        self.eps = float(eps)

    def forward(self, xy: torch.Tensor):
        """
        xy: [B,2]  -> returns (xy_trans: [B,2], w: [B,1])
        """
        B = xy.shape[0]
        ones = torch.ones(B, 1, device=xy.device, dtype=xy.dtype)
        homo = torch.cat([xy, ones], dim=-1)         # [B,3]
        out  = homo @ self.H.t()                     # [B,3]
        w    = out[:, 2:3]
        den  = w.sign() * torch.clamp(w.abs(), min=self.eps)  # keep sign, avoid 0
        xy_t = out[:, :2] / den
        return xy_t, w
...
def train_student_uniform_xt(cfg: Dict):
    ...
    for step_i in range(1, total_steps + 1):
        ...
        xt_S, w_S = H_module(xt_S)   # [B,2], [B,1]
        # xt_T, w_T = H_module(xt_T)   # [B,2], [B,1]
        ...
        if cfg.get("use_wandb", False) and ((step_i % cfg["vis_interval_epochs"] == 0) or (step_i == total_steps)):
            with torch.no_grad():
                ...
                # 1) 현재 H
                H_t: torch.Tensor = H_module.H.detach()


                # 3) 정규화 (proj. scale 제거)
                H_t_n = H_t / H_t[2, 2].clamp(min=eps)


                # 6) HTML (아주 심플: 테이블+pre)
                def mat_str(T):
                    arr = T.detach().float().cpu().numpy()
                    return html.escape(np.array2string(arr, formatter={'float_kind': lambda x: f"{x: .5f}"},
                                                    max_line_width=200))

                html_block = f"""
                <table>
                <tr>
                    <th style="text-align:center;">Pred inv H</th>
                    <th style="text-align:center;">Pred inv H (norm)</th>
                </tr>
                <tr>
                    <td><pre>{mat_str(H_t)}</pre></td>
                    <td><pre>{mat_str(H_t_n)}</pre></td>
                </tr>
                </table>
                """.strip()
