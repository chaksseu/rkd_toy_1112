...
WANDB_NAME=f"1119_lr1e4_n32_b{BATCH_SIZE}_ddim_50_150_steps_no_init_rkdW{W_RKD}_xt"
...
CONFIG = {
    ...
    # wandb
    "use_wandb": True,
    "wandb_project": "RKD-DKDM-AICA-1119",
...
def train_student_uniform_xt(cfg: Dict):
    ...
    for step_i in range(1, total_steps + 1):
        ...
        xt_T_seq = torch.stack(xt_T_seq, dim=0)
        xt_T_seq_flat = xt_T_seq.reshape(-1, xt_T_seq.shape[-1])

        xt_S_seq_flat = denormalize_torch(xt_S_seq_flat, mu_student_tensor, sigma_student_tensor)
        xt_T_seq_flat = denormalize_torch(xt_T_seq_flat, mu_teacher_tensor, sigma_teacher_tensor)

        rkd_s_d = torch.pdist(xt_S_seq_flat, p=2).clamp_min(1e-12)           
        rkd_t_d = torch.pdist(xt_T_seq_flat, p=2).clamp_min(1e-12)  

        # student mean
        student_sum = rkd_s_d.sum()
        student_cnt = rkd_s_d.numel()
        student_mean = student_sum / student_cnt  
        # teacher mean
        teacher_sum = rkd_t_d.sum() 
        teacher_cnt = rkd_t_d.numel()
        ...
        if (step_i % 5 == 0) or (step_i == 1):
            print(f"[step {step_i:06d}] rkd={rkd_loss.item():.6f} total={loss.item():.6f}")
