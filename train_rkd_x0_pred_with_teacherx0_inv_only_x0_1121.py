...
def train_student_uniform_xt(cfg: Dict):
    ...
    for step_i in range(1, total_steps + 1):
       ...
       ##################  RKD LOSSES ##################
        ...
        xt_T_seq_denorm = [denormalize_torch(x, mu_teacher_tensor, sigma_teacher_tensor) for x in xt_T_seq]
        ...
        if cfg["W_RKD"] != 0:
            for xt_S, xt_T in zip(xt_S_seq_denorm, xt_T_seq_denorm):
        ...
        if cfg["W_INV"] != 0:
            ...
            t_full = torch.cdist(xt_T_seq_denorm[-1], x0_inv_T_denorm, p=2)
        ...
        if cfg["W_FID"] != 0:
            ...
            fid_teacher = fid_gaussian_torch(xt_T_seq_denorm[-1], x0_inv_T_denorm)
        else:
