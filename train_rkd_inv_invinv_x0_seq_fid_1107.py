...
WANDB_NAME=f"1107_lr1e4_n32_b{BATCH_SIZE}_ddim_50_150_steps_no_init_rkdW{W_RKD}_invW{W_INV}_invinvW{W_INVINV}_fidW{W_FID}_onlyX0"
...
def train_student_uniform_xt(cfg: Dict):
    ...
    for step_i in range(1, total_steps + 1):
        ...
        for i, (xt_S, xt_T, xt_S_inv, xt_T_inv) in enumerate(zip(xt_S_seq, xt_T_seq, reversed(S_inv_z_seq[:-1]), x0_inv_T)):
            ...
            inversion_loss += cfg["W_INV"] * F.mse_loss(inv_s_d, inv_t_d, reduction="mean") #/Slen(xt_S_seq)
        ...
        if cfg["use_wandb"]:
            wandb.log({
                "loss/rkd_raw": float(rkd_loss) / cfg["W_RKD"] if cfg["W_RKD"] != 0 else 0.0,
                "loss/inv_raw": float(inversion_loss) / cfg["W_INV"] if cfg["W_INV"] != 0 else 0.0,
                "loss/invinv_raw": float(invinv_loss) / cfg["W_INVINV"] if cfg["W_INVINV"] != 0 else 0.0,
                "loss/fid_raw": float(fid_loss) / cfg["W_FID"] if cfg["W_FID"] != 0 else 0.0,
                "loss/diff_loss": float(diff_loss) / cfg["W_DIFF"] if cfg["W_DIFF"] != 0 else 0.0
            }, step=step_i)
